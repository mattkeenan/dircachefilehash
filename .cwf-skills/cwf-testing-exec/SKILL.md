...
- Construct changeset: run `.cwf/scripts/command-helpers/security-review-changeset --phase=testing --max-lines=500`, capturing **both stdout and the exit code** (and stderr, for the cap reason). The helper resolves the anchor, emits the full diff over all changed files per § "Changeset coverage", and enforces the production-weighted review cap. Branch on the exit code:
