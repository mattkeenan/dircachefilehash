...
**Task 29 (v1.1.189→v1.1.201 upgrade) is the fourth data point.** Both exec phases exited 2 (`1384 production lines > 500`) on the relaid `.cwf/**` delta — consistent with Tasks 5/9/14. Handling differed from Task 14: rather than raise the cap and spend a full subagent pass on upstream-vetted code, Task 29 built a **55-line inter-repo-only changeset** (`.claude/settings.json` + `.gitignore` + `CLAUDE.md`) and reviewed *that* with `cwf-security-reviewer-changeset` → `no findings`, recording the mechanical exit-2 verbatim alongside the scoped verdict. This is the cheapest defensible option for a *pure* upgrade (it reviews exactly the consumer-authored surface) and reinforces the standing decision: reserve any `.cwf/**` exclude for *mixed* tasks; pure upgrades are better served by the scoped review + the deterministic `cwf-manage validate` integrity gate than by routing the whole vendored delta through a subagent.
