...
func benchmarkDirectoryScan(b *testing.B, config BenchmarkConfig) {
	...
	for i := 0; i < b.N; i++ {
		...
		if err := runUpdate(context.Background(), cache, cache.scanRun(), map[string]string{}); err != nil {
...
func benchmarkIndexOperations(b *testing.B, config BenchmarkConfig) {
	...
	b.Run("Status", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			cache := NewMetaStore(datasetDir, dcfhDir)
			if _, err := runStatus(context.Background(), cache, cache.scanRun(), map[string]string{}, nil); err != nil {
	...
	b.Run("FindDuplicates", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			...
			if _, err := runFindDuplicates(context.Background(), cache, cache.scanRun(), map[string]string{}, DupeFilter{Exclusive: true}); err != nil {
...
func BenchmarkFullWorkflowMedium(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		statusResult, err := runStatus(context.Background(), cache, cache.scanRun(), map[string]string{"v": "1"}, nil)
		if err != nil {
			b.Fatalf("Status failed: %v", err)
		}
