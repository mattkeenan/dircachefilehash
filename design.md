...
This index is saved in a `.dcfh` directory (in a similar fashion to git) and the current index file is called `index` in the `.dcfh` directory. A cache index file called `index.cache` is a sparse index file that may or may not be in pathname order on disk (but ordered via a skiplist after it is loaded; see index file format below for more details) and is used to temporarily cache any changes on disk that haven't been updated into the main `index` file; e.g. via the `dcfh update` command. Temporary index files are called `index-<pid>-<tid>.tmp` (process id and thread id) to make sure there are no collisions. The `.dcfh` directory also has an `ignore` file that uses golang `regexp` to ignore, or to unignore a subset of an ignore, files from the index(es).
...
A copy of the main index skiplist is made, this copy is then merged with the on disk sparse `index.cache`, and then a scan of all the files and directories is made in a temporary sparse index file, and then the skiplist for this temporary sparse index file is merged into the initial copy of the skiplist, and then this updated copy of the skiplist (which now has up to date entries for all the files) is then used via `Pwritev()` to efficiently write all the entries (using gather writes from the various source entries in the old full index, cache index, and update index files) into a new in order full index. 
...
2. Merge with the existing `index.cache` skiplist if it exists  
